...
import webservice.Unite;

/**
 *
 * @author antho
 */
public class AboutToTransactionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{
                ...
                List<Banque> banquesTransa = banque.findBanqueTransaction(search);
                List<Personne> personnes = Personne.findPersonne(search);
                List<Unite> unites = Unite.selectAll();
                request.setAttribute("personnes", personnes);
                request.setAttribute("banques", banquee);
                request.setAttribute("unites", unites);
                request.setAttribute("banquesTransa", banquesTransa);
                 try{
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AboutToTransactionServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AboutToTransactionServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
